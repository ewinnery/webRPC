...
2. Запустите `webrpc ready build.exe`.
...
3. Нажмите **Загрузить распакованное расширение** и выберите папку `extension src`.
...
- **Расширение показывает Disconnected:** Проверьте, запущен ли `webrpc ready build.exe` (клиент использует локальный порт `8765`).
